...
**Interpretation:**OOB error should dropped quickly and then plateaued. And the dashed red line is markig the final OOB value. The mtry sweep was a near-tie: the algorithmic best was mtry=`r rf_tuning$mtry[which.min(rf_tuning$OOB_RMSE)]` at OOB RMS `r round(min(rf_tuning$OOB_RMSE), 3)`, but `r sum(rf_tuning$within_1pct_of_best == "✓")` of 5 values landed within 1% of the best, so we went with ISLP's default mtry=7 (RMSE `r round(rf_tuning$OOB_RMSE[rf_tuning$mtry == 7], 3)`). That mtry=2 (essentially bagging) is only marginally worse tells us feature decorrelation isn't doing heavy lifting on this dataset. The OOB-vs-ntree curve dropped fast from `r round(oob_curve$oob_rmse[1], 1)` at ntree=1 to `r round(oob_curve$oob_rmse[200], 2)` by ntree=200, then flatlines.  So ntree=500 is well past the plateau and definitely enough. We could even cut bak to ~300 trees, but 500 is pretty standard so we kept it.
...
**Interpretation:** Distribution of individual treatment effects (ITE) across the *treated* tracts. The red vertical line is the ATT (average across treated). Negative ITE values = treatment reduces long-commute % (i.e. helps). All-negative = treatment uniformly helps every treated tract. Fortuantely, this plot looks all negative! (Though the magnitude is quite small.)
...
**Interpretation: ** Okay we figured it out! Negative bar = feature is associated with bigger benefit. The boroughQueens bar (negative) means Queens treated tracts benefit more than Brooklyn treated tracts (becuase Brooklyn is the reference). 
