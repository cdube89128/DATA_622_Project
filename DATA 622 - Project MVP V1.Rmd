...
lm_fit_eval <- lm(pct_commute_60p ~ . - GEOID, data = train_data)

## Train RF on train set (for evaluation)
rf_fit_eval <- randomForest(
  pct_commute_60p ~ . - GEOID,
  data       = train_data,
  ntree      = 500,
  importance = TRUE
)
...
```{r}
ps_model <- glm(z ~ ., data = BART_df, family = binomial)
...
BART_df <- data.frame(ps = ps, z = z, y =y, z=z, x=x)
BART_df <- na.omit(df)
...
```{r}
bal_ate <- bal.tab(x, treat = z, weights = w_ate, estimand = "ATE",un = TRUE,quick = FALSE)
bal_att <- bal.tab(x, treat = z, weights = w_att, estimand = "ATT",un = TRUE,quick = FALSE)
bal_atc <- bal.tab(x, treat = z, weights = w_atc, estimand = "ATC",un = TRUE,quick = FALSE)
```
...
#tracts_sf <- st_as_sf( WHAT DF SHOULD THIS BE?)
...
```{r}
spatial_lag_df$lag_income <- lag.listw(weights, BART_df$med_income)
