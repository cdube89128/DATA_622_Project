...
# The minus sign (- GEOID) explicitly tells the model NOT to use the ID as a predictor
lm_fit_standardized <- lm(bike_index ~ . - GEOID, data = ml_data)

# 3. Fit the "George MVP" Simple Model
lm_fit_simple <- lm(bike_index ~ dist_subway_miles, data = ml_data)
...
# 6. Run the Predictions, Calculate Surge, AND Flag Station Locations
...
```{r model-outputs}
# Load broom for tidy model outputs (dotwhisker removed since you built a great custom ggplot!)
pacman::p_load(broom)

# Turn the STANDARDIZED model results into a clean table
...
ggplot(lm_results, aes(x = estimate, y = reorder(term, estimate))) +
  ...
  theme_minimal() +
  labs(title = "Feature Importance: What drives the 'Bike Index'?",
       subtitle = "Standardized Coefficients (Variables further from 0 have a stronger impact)",
       x = "Effect on Bike Index (per 1 Std. Dev. increase)",
       y = "Feature")
```
