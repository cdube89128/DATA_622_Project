...
**Interpretation:** Random Forest wins clearly; it's the only model within 1 SE of the best, and each step up (linear → spline → RF) gains predictive power. The spline-to-RF gap (`r round(cv_table$RMSE_mean[3] - cv_table$RMSE_mean[2], 2)` RMSE) is bigger than the lm-to-spline gap (`r round(cv_table$RMSE_mean[2] - cv_table$RMSE_mean[1], 2)`), meaning RF is catching nonlinearities and interactions the spline alone can't. RF's typical prediction is off by ~`r round(cv_table$RMSE_mean[3], 1)`pp on the 0-100% scale and explains ~`r round(cv_table$R2_mean[3] * 100)`% of variance. For the IBX simulation, Models 2 and 3 are better to lean on. Model 1 can't capture the saturating subway-access effect, so its predicted relief is essentially zero.
