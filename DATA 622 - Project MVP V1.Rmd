...
**Interpretation:** Random Forest + Spatial Lags is the CV-best by just a little bit, with `r round(cv_table$RMSE_mean[6], 2)` RMSE vs `r round(cv_table$RMSE_mean[5], 2)` for plain Random Forest. The two Random Forest variants are the only rows within 1 SE of the best, so they're statistically a tie, but we're opting to treat Random Forest + Spatial Lags as the winner. Its typical prediction is off by ~`r round(cv_table$RMSE_mean[6], 1)`pp on the 0-100% scale, and it explains ~`r round(cv_table$R2_mean[6] * 100)`% of variance. There are also two patterns in the table that are worth pointing at. Across the spatial-lag axis, every model class gains roughly 0.10 to 0.15 RMSE once neighbor-weighted features are added (Linear Regression `r round(cv_table$RMSE_mean[1], 2)` to `r round(cv_table$RMSE_mean[2], 2)`, Linear + Spline `r round(cv_table$RMSE_mean[3], 2)` to `r round(cv_table$RMSE_mean[4], 2)`, Random Forest `r round(cv_table$RMSE_mean[5], 2)` to `r round(cv_table$RMSE_mean[6], 2)`). The improvement is consistent across all three classes. Basically, neighbors are carrying real signal beyond what a tract's own features capture. Across the model-family axis, the jump from Linear + Spline to Random Forest (`r round(cv_table$RMSE_mean[5] - cv_table$RMSE_mean[3], 2)` RMSE) is bigger than the jump from Linear Regression to Linear + Spline (`r round(cv_table$RMSE_mean[3] - cv_table$RMSE_mean[1], 2)`), meaning Random Forest is catching nonlinearities and interactions the spline alone can't. For the IBX simulation, the Linear + Spline, Random Forest, and spatial-lag variants are the ones to lean on. Plain Linear Regression can't capture the saturating subway-access effect, so its predicted relief is essentially zero. The spatial-lag variants also let predicted relief spill over to tracts next to the new corridor, so they reach more tracts than the non-spatial versions (more on that in the IBX simulation section below).
