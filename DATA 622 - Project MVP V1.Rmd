...
### Model 1: Linear Regression (with and without spline)
...
Both fit on a **logit-transformed outcome** so predictions stay in [0, 100] (without it, plain `lm` will happily predict -3% or 105% when IBX pushes one feature to an extreme).

The spline version handles the saturating distance effect George flagged — closer matters a lot, far doesn't (ISLP §7.4 prescribes natural cubic splines for this case). Linear baseline kept alongside so the Shiny app can show both.

Heavy-tailed predictors (`uber_trips_per_capita`, `bike_trips_per_capita`, `stations_per_1000_pop`, `pop_density`) get a `log1p()` transform before z-scoring — same idea, also George's flag. After logging, a 1 SD change in the standardized predictor is an order-of-magnitude change rather than an absolute one, which is the right scale for these variables.
...
### Model 2: Random Forest + 10-fold Cross-Validation

We compare three models — pure linear, linear+spline, and Random Forest — using 10-fold CV stratified by borough. Same folds for every model so the RMSE/R² comparisons are paired (ISLP Ch. 5 / Meetup 6). The **one-SE rule** then picks among them.

A single 80/20 split — what we had before — is the high-variance case ISLP §5.1.1 explicitly warns against. CV averages out the fold-level noise.

`rf_fit` is fit on the full dataset (no holdout) for the IBX simulation downstream — separate from the CV loop.
...
# comparisons are paired (per ISLP Ch. 5 / Meetup 6).
set.seed(622)
...
## SUMMARIZE — mean ± SE across folds, plus the one-SE rule flag
...
#### RF Hyperparameter Tuning (mtry sweep, OOB)

Per ISLP §8.2.1 and Meetup 8: with enough trees, OOB error is virtually equivalent to LOOCV — so we tune `mtry` by reading `rf$mse` off the full-data fit, no separate CV needed. ISLP's default is `mtry ≈ p/3` ≈ 7 for our 22 predictors; the sweep tests neighbors and the bagging endpoint (`mtry = 22`). We also verify `ntree = 500` is past the OOB plateau.

If multiple `mtry` values come back within 1% of the OOB-best, we go with the **ISLP default** (`p/3 ≈ 7`) — the one-percent-rule analogue. Easier to defend than the algorithmic best when the spread is within Monte Carlo noise.
