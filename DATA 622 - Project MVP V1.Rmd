...
# Drop GEOID before splitting. It's a per-row unique ID, not a predictor —
# every tract is its own factor level, so any holdout will contain levels
# unseen in train and predict() will error with "factor GEOID has new levels".
# `- GEOID` in the formula isn't enough; R retains it in the model frame.
model_data <- ml_data_scaled[, c(target_var, predictor_vars)]

train_idx  <- sample(nrow(model_data), size = 0.8 * nrow(model_data))
train_data <- model_data[train_idx, ]
test_data  <- model_data[-train_idx, ]

## Retrain linear on train set only (for a fair apples-to-apples comparison)
lm_fit_eval <- lm(pct_commute_60p ~ ., data = train_data)

# Same with the spline on dist_subway_miles, for the eval-table comparison.
lm_fit_spline_eval <- lm(pct_commute_60p ~ . - dist_subway_miles
                         + splines::ns(dist_subway_miles, df = 4),
                         data = train_data)
...
lm_preds        <- predict(lm_fit_eval,        newdata = test_data)
lm_spline_preds <- predict(lm_fit_spline_eval, newdata = test_data)
rf_preds        <- predict(rf_fit_eval,        newdata = test_data)
...
eval_table <- tibble(
  Model = c("Linear Regression", "Linear + Spline (dist_subway_miles)", "Random Forest"),
  RMSE  = round(c(rmse(actuals, lm_preds), rmse(actuals, lm_spline_preds), rmse(actuals, rf_preds)), 3),
  R2    = round(c(r2(actuals,   lm_preds), r2(actuals,   lm_spline_preds), r2(actuals,   rf_preds)), 3)
)
